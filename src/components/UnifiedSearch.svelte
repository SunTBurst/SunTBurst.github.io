<script lang="ts">
  ...
  import { decideUnifiedSearchKey, matchesPortalSearch, resolveSearchActiveIndex } from '../utils/unifiedSearchCore';
  ...
  $: results = normalized
    ...
    : entries;
  $: activeIndex = resolveSearchActiveIndex(focusedIndex, hoveredIndex, results.length);
  ...
  function focusSearch(event: Event) {
    if (tryFocusPortalSearch(input, () => document.activeElement)) event.preventDefault();
  }
  ...
  function handleKeydown(event: KeyboardEvent) {
    const decision = decideUnifiedSearchKey(event, focusedIndex, results.length);
      ...
      {#each results as result, index}
        <li class={`min-w-0 border-2 ${index === activeIndex ? 'border-[#f59e0b]' : 'border-[#0284c7]'} bg-[#f8fafc] p-3 dark:bg-slate-900`}>
          <a
            ...
            on:blur={() => { if (focusedIndex === index) focusedIndex = -1; }}
            on:mouseenter={() => hoveredIndex = index}
            on:mouseleave={() => { if (hoveredIndex === index) hoveredIndex = -1; }}
