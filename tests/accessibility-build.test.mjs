...
test('portal provides a keyboard bypass and keeps active navigation text at AA contrast', () => {
  ...
  const activeHome = html.match(/<a\b(?=[^>]*data-desktop-primary)(?=[^>]*aria-current="page")[^>]*>/)?.[0] ?? '';
...
test('article calendar and metadata remain perceivable and comfortably operable', () => {
  const html = readFileSync(path.join(distDir, 'posts', 'hello-world', 'index.html'), 'utf8');
  const calendar = html.match(/<(?:section|div)\b[^>]*data-calendar-widget[^>]*>/)?.[0] ?? '';
  assert.ok(calendar, 'expected the article calendar landmark');
  ...
  const monthButtons = tagsWithMarker(html, 'data-calendar-month');
  ...
  assert.equal(tagsWithMarker(html, 'data-calendar-day').length > 27, true, 'expected non-interactive semantic day cells');
  assert.equal(tagsWithMarker(html, 'data-calendar-day-button').length, 0, 'expected empty dates not to create dozens of tab stops');
